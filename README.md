...
flowchart TD
    ...
    CHOOSE --> COMPAT{"Compatible with Agent-Team 7.2.5?"}
    COMPAT -->|Yes| SEED
...
flowchart LR
    ...
    USER -->|another tracker| NAT["Use it only without Agent-Team 7.2.5"]
...
For Agent-Team 7.2.5, the handoff tracker is Beads or Markdown at `TASKS.md` or
`.agent-team/TASKS.md`. Another tracker can support a project that does not use
Agent-Team, but it is not a compatible Agent-Team 7.2.5 handoff target.
...
| `.project-kickoff/AGENT_TEAM_HANDOFF.json` | Gives Agent-Team 7.2.5 a bounded, validated, machine-readable initialization input. |
...
prevents Agent-Team 7.2.5 from claiming an open summary record as implementation
...
`scripts/check_agent_team_handoff.py`. The checker enforces the 7.2.5 boundary:
...
Beads validation uses the same five-second read window as Agent-Team 7.2.5. A
