...
def valid_handoff(root):
    ...
    return {
        ...
        "agentTeam": {"testedVersion": "7.2.5", "initializationSource": "existing"},
        "project": {
            "id": "fixture-project",
            "root": str(root),
            "branch": "main",
...
class AgentTeamHandoffTests(unittest.TestCase):
    ...
    def test_package_declares_the_machine_readable_handoff(self):
        ...
        self.assertEqual(template["agentTeam"]["testedVersion"], "7.2.5")
    ...
    def test_emit_request_rebinds_the_current_descendant_tip(self):
        ...
        self.assertEqual(binding, {
            ...
            "testedAgainst": {"name": "agent-team", "version": "7.2.5"},
    ...
    def test_preliminary_local_agent_team_725_consumes_the_042_handoff(self):
        agent_team = Path(os.environ["AGENT_TEAM_ROOT"]).resolve()
        version = (agent_team / "SKILL.md").read_text()
        self.assertIn('version: "7.2.5"', version)
